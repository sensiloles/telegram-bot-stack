...
bot_path = project_root / "examples" / "menu_bot" / "bot.py"


class TestMenuBot:
    """Test suite for Menu Bot."""

    @pytest.fixture
    def bot_module(self):
        """Load menu bot module."""
        bot_dir = bot_path.parent
        if str(bot_dir) not in sys.path:
            sys.path.insert(0, str(bot_dir))

        spec = importlib.util.spec_from_file_location("menu_bot_module", bot_path)
        module = importlib.util.module_from_spec(spec)
        sys.modules["menu_bot_module"] = module
    ...
    def bot_class(self, bot_module):
        """Get MenuBot class."""
        return bot_module.MenuBot

    @pytest.fixture
    def bot_instance(self, bot_class):
        """Create MenuBot instance with memory storage."""
        storage = create_storage("memory")
        return bot_class(storage=storage, bot_name="Test Menu Bot")

    def test_module_import(self, bot_module):
        """Test that menu bot module can be imported."""
        assert bot_module is not None

    def test_class_exists(self, bot_class):
        """Test that MenuBot class exists."""
        assert bot_class is not None
        assert bot_class.__name__ == "MenuBot"

    def test_initialization(self, bot_instance):
        """Test that MenuBot can be initialized."""
        assert bot_instance is not None
        assert bot_instance.bot_name == "Test Menu Bot"
    ...
    def test_has_storage_keys(self, bot_instance):
        ...
        assert hasattr(bot_instance, "SETTINGS_KEY")
        assert hasattr(bot_instance, "ITEMS_KEY")
        assert bot_instance.SETTINGS_KEY == "user_settings"
        assert bot_instance.ITEMS_KEY == "items"
