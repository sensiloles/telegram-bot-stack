...
def up(config: str, verbose: bool) -> None:
    ...
    try:
        ...
        try:
            ...
            # Decrypt secrets on VPS and create .secrets.env for Docker Compose
            # This ensures encrypted secrets stay encrypted on VPS filesystem
            # and are only decrypted in-memory for container use
            encryption_key = deploy_config.get("secrets.encryption_key")
            if encryption_key:
                secrets_manager = SecretsManager(bot_name, remote_dir, encryption_key)
                decrypted_secrets = secrets_manager.load_secrets_to_env(vps)

                if decrypted_secrets:
                    # Create decrypted .secrets.env on VPS for Docker Compose
                    # Encrypted secrets remain in .secrets.env.encrypted (managed by SecretsManager)
                    # Decrypted version is created on VPS, not transferred from local
                    decrypted_file = f"{remote_dir}/.secrets.env"
                    secrets_content = "# Decrypted secrets for Docker\n"
                    secrets_content += (
                        "# Generated by telegram-bot-stack at deployment time\n"
                    )
                    secrets_content += "# Source: encrypted .secrets.env.encrypted (decrypted in-memory)\n"
                    secrets_content += (
                        "# WARNING: This file contains plain text secrets!\n"
                    )
                    secrets_content += (
                        "# Encrypted version preserved in .secrets.env.encrypted\n\n"
                    )
                    for key, value in sorted(decrypted_secrets.items()):
                        secrets_content += f"{key}={value}\n"

                    # Write decrypted version directly to VPS
                    if vps.write_file(secrets_content, decrypted_file, mode="600"):
                        console.print(
                            "[green]✓ Decrypted secrets for Docker container[/green]"
                        )
                        console.print(
                            "[dim]   (Encrypted version preserved in .secrets.env.encrypted)[/dim]"
                        )
                    else:
                        console.print(
                            "[yellow]⚠️  Warning: Could not create decrypted secrets file[/yellow]"
                        )
