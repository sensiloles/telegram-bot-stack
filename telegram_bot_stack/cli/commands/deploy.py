...
def up(config: str, verbose: bool) -> None:
    ...
    try:
        ...
        # Decrypt and create .secrets.env file for Docker if secrets exist
        ...
        if encryption_key:
            ...
            if decrypted_secrets:
                # Create .secrets.env file with decrypted secrets
                secrets_env_file = temp_dir / ".secrets.env"
                secrets_content = "# Decrypted secrets for Docker\n"
                secrets_content += "# Generated by telegram-bot-stack\n\n"
                for key, value in sorted(decrypted_secrets.items()):
                    secrets_content += f"{key}={value}\n"
                secrets_env_file.write_text(secrets_content)
                console.print(
                    "[green]✓ Generated .secrets.env from encrypted secrets[/green]"
                )
